...
const PRODUCTS = [
  ...
  { id: "boite-pizza", nom: "Boîte pizza", nomEn: "Pizza box", cat: "kraft" },
...
const PRODUCT_IMAGES = {
  ...
  "boite-pizza": "assets/products/boite-pizza.jpg",
...
/* ---------------------------------------------------------------------- */
/* COMMANDE FOURNISSEUR (PDF bilingue) */
/* ---------------------------------------------------------------------- */

const ORDER_I18N = {
  fr: {
    docTitle: "Bon de commande fournisseur", docSub: "Liste des produits, formats et quantités à commander",
    date: "Date", supplier: "Fournisseur", ref: "Référence", product: "Produit", size: "Format",
    qty: "Quantité", total: "Total", totalUnits: "unités au total", noSupplier: "Non précisé",
    tailles: { Petit: "Petit", Moyen: "Moyen", Grand: "Grand" },
    footer: "Document généré par l'application de gestion Teranga Packaging.",
  },
  en: {
    docTitle: "Supplier Purchase Order", docSub: "List of products, sizes and quantities to order",
    date: "Date", supplier: "Supplier", ref: "Reference", product: "Product", size: "Size",
    qty: "Quantity", total: "Total", totalUnits: "units total", noSupplier: "Not specified",
    tailles: { Petit: "Small", Moyen: "Medium", Grand: "Large" },
    footer: "Document generated by the Teranga Packaging management app.",
  },
};

let ordLang = "fr";
let ordActiveCat = "kraft";
let ordLines = [];

function renderCommande() {
  if (!document.getElementById("ord-date").value) document.getElementById("ord-date").value = todayInput();
  renderOrdPicker();
  renderOrdLines();
}

function renderOrdPicker() {
  const grid = document.getElementById("ordGrid");
  const items = PRODUCTS.filter(p => p.cat === ordActiveCat);
  grid.innerHTML = items.map(p => {
    const already = ordLines.some(l => l.produitId === p.id);
    return `<button type="button" class="pkg-item${already ? " checked" : ""}" data-id="${p.id}">
      ${productThumb(p.id, 46)}
      <span class="pkg-item-name">${escapeHtml(p.nom)}</span><span class="pkg-item-check">${already ? "✓ ajouté" : "+ ajouter"}</span>
    </button>`;
...
function renderOrdLines() {
  ...
  if (!ordLines.length) {
    wrap.innerHTML = '<p class="empty-hint">Aucun produit ajouté. Cliquez sur un produit ci-dessus pour l\'ajouter à la commande.</p>';
    return;
  }
  wrap.innerHTML = ordLines.map((l, i) => {
    const allChecked = TAILLES.every(t => l.tailles[t].checked);
    return `
    <div class="pkgline" data-idx="${i}">
      <div class="pkgline-top">
        ...
        <span class="pkgline-name">${escapeHtml(productName(l.produitId))}</span>
        <label class="pkgline-all">
          <input type="checkbox" class="ordline-allcheck" data-idx="${i}" ${allChecked ? "checked" : ""}>
          Toutes les tailles
        </label>
        <button type="button" class="icon-btn del ordline-del" data-idx="${i}" title="Retirer">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
        </button>
      </div>
      <div class="pkgline-tailles">
        ${TAILLES.map(t => `
          <label class="taille-chip${l.tailles[t].checked ? " checked" : ""}">
            <input type="checkbox" class="ordline-taillecheck" data-idx="${i}" data-taille="${t}" ${l.tailles[t].checked ? "checked" : ""}>
            <span>${t}</span>
            <input type="number" min="1" step="1" class="ordline-tailleqty" data-idx="${i}" data-taille="${t}" value="${l.tailles[t].quantite}" ${l.tailles[t].checked ? "" : "disabled"}>
          </label>`).join("")}
      </div>
    </div>`;
  }).join("");
...
function generateOrderPdf() {
  if (!ordLines.length) { toast("Ajoutez au moins un produit à la commande.", "error"); return; }
  const t = ORDER_I18N[ordLang];
  const rows = ordLines.flatMap(l => TAILLES.filter(tl => l.tailles[tl].checked).map(tl => ({
    produitId: l.produitId, taille: tl, quantite: Number(l.tailles[tl].quantite) || 1,
  })));
  if (!rows.length) { toast("Sélectionnez au moins un format par produit.", "error"); return; }

  const totalUnits = rows.reduce((s, r) => s + r.quantite, 0);
  const dateStr = new Date(document.getElementById("ord-date").value || todayInput()).toLocaleDateString(
    ordLang === "en" ? "en-US" : "fr-FR", { day: "2-digit", month: "short", year: "numeric" }
  );
  const fournisseur = document.getElementById("ord-fournisseur").value.trim() || t.noSupplier;
  const ref = document.getElementById("ord-ref").value.trim();
  const productLabel = (id) => { const p = findProduit(id); return p ? (ordLang === "en" ? p.nomEn : p.nom) : id; };
  const tailleLabel = (taille) => t.tailles[taille] || taille;

  const rowsHtml = rows.map(r => `
    <tr>
      <td><img src="${PRODUCT_IMAGES[r.produitId] || ""}" alt=""></td>
      <td>${escapeHtml(productLabel(r.produitId))}</td>
      <td>${tailleLabel(r.taille)}</td>
      <td>${r.quantite.toLocaleString(ordLang === "en" ? "en-US" : "fr-FR")}</td>
    </tr>`).join("");

  const logoSrc = document.querySelector(".side-brand img")?.src || "icons/icon-192.png";
  ...
  document.getElementById("printArea").innerHTML = `
    <div class="pdf-doc">
      <div class="pdf-head">
        <div class="pdf-brand">
          <img src="${logoSrc}" alt="">
          <div><strong>TERANGA PACKAGING</strong><span>DAKAR · SÉNÉGAL</span></div>
        </div>
        <div class="pdf-contact">
          +221 76 966 31 94<br>
          contact@terangapackaging.sn<br>
          Cité Apix, Lot 2, Dakar, Sénégal
        </div>
      </div>

      <div class="pdf-title">${t.docTitle}</div>
      <div class="pdf-sub">${t.docSub}</div>

      <div class="pdf-meta">
        <div><strong>${t.date} :</strong> ${dateStr}</div>
        <div><strong>${t.supplier} :</strong> ${escapeHtml(fournisseur)}</div>
        ${ref ? `<div><strong>${t.ref} :</strong> ${escapeHtml(ref)}</div>` : ""}
      </div>

      <table class="pdf-table">
        <thead><tr><th></th><th>${t.product}</th><th>${t.size}</th><th>${t.qty}</th></tr></thead>
        <tbody>${rowsHtml}</tbody>
      </table>

      <div class="pdf-total">${t.total} : ${totalUnits.toLocaleString(ordLang === "en" ? "en-US" : "fr-FR")} ${t.totalUnits}</div>

      <div class="pdf-foot">${t.footer}</div>
    </div>
  `;
...
function bindEvents() {
  ...
  document.getElementById("ordLang").addEventListener("click", (e) => {
    const btn = e.target.closest(".seg-btn");
    if (!btn) return;
    ordLang = btn.dataset.lang;
    document.querySelectorAll("#ordLang .seg-btn").forEach(b => b.classList.toggle("active", b === btn));
  });
  document.getElementById("ordTabs").addEventListener("click", (e) => {
    const btn = e.target.closest(".pkg-tab");
    if (!btn) return;
    ordActiveCat = btn.dataset.cat;
    document.querySelectorAll("#ordTabs .pkg-tab").forEach(t => t.classList.toggle("active", t === btn));
    renderOrdPicker();
  });
  document.getElementById("ordGrid").addEventListener("click", (e) => {
    const btn = e.target.closest(".pkg-item");
    if (!btn) return;
    const id = btn.dataset.id;
    if (ordLines.some(l => l.produitId === id)) { toast("Ce produit est déjà dans la commande."); return; }
    ordLines.push(newPkgLine(id));
    renderOrdPicker();
    renderOrdLines();
  });
  document.getElementById("ordLines").addEventListener("input", (e) => {
    const qtyInput = e.target.closest(".ordline-tailleqty");
    if (qtyInput) ordLines[Number(qtyInput.dataset.idx)].tailles[qtyInput.dataset.taille].quantite = Number(qtyInput.value) || 1;
  });
  document.getElementById("ordLines").addEventListener("change", (e) => {
    const tailleCheck = e.target.closest(".ordline-taillecheck");
    const allCheck = e.target.closest(".ordline-allcheck");
    if (tailleCheck) {
      const idx = Number(tailleCheck.dataset.idx);
      ordLines[idx].tailles[tailleCheck.dataset.taille].checked = tailleCheck.checked;
      if (TAILLES.every(tl => !ordLines[idx].tailles[tl].checked)) {
        toast("Sélectionnez au moins un format pour ce produit.", "error");
        ordLines[idx].tailles[tailleCheck.dataset.taille].checked = true;
      }
      renderOrdLines();
    }
    if (allCheck) {
      const idx = Number(allCheck.dataset.idx);
      TAILLES.forEach(tl => { ordLines[idx].tailles[tl].checked = allCheck.checked; });
      renderOrdLines();
    }
  });
  document.getElementById("ordLines").addEventListener("click", (e) => {
    const delBtn = e.target.closest(".ordline-del");
    if (delBtn) {
      ordLines.splice(Number(delBtn.dataset.idx), 1);
      renderOrdPicker();
      renderOrdLines();
    }
  });
  document.getElementById("genPdfBtn").onclick = generateOrderPdf;
